...
function App() {
  ...
  const [newWorker, setNewWorker] = useState({ username: '', password: '', centro_id: '' })
  ...
  async function login(e) {
    e.preventDefault()
    const email = loginIdToEmail(loginUser)
  ...
  async function crearTrabajador() {
    const username = cleanUsername(newWorker.username)
    if (!username || !newWorker.password || !newWorker.centro_id) {
      alert('Rellena usuario, contraseña y centro.')
      return
    }
    ...
    const res = await fetch('/api/create-worker', {
      ...
      body: JSON.stringify({
        username,
        email: `${username}@${workerDomain}`,
        password: newWorker.password,
        nombre: username,
        centro_id: newWorker.centro_id
      })
    })
    ...
    if (!res.ok) {
      alert(data.error || 'No se pudo crear el trabajador.')
      return
    }

    setNewWorker({ username: '', password: '', centro_id: centros[0]?.id || '' })
    alert(`Trabajador creado. Usuario: ${username}`)
    await loadAll()
  }
  ...
  if (!session || !profile) {
    return <div className="login">
      <form className="card login-card" onSubmit={login}>
        ...
        <p className="muted small">El superadmin entra con email. El trabajador entra con el usuario creado por el superadmin.</p>
      </form>
    </div>
  }
  ...
  return <div>
    <header>
      <div>
        <h1>Korvantis Fresh Product Control</h1>
        <p>{isAdmin ? 'Superadmin · Vista global' : `Trabajador · ${profile.nombre || profile.email?.replace('@korvantis.local','')}`}</p>
      </div>
      <div className="row">
        <button onClick={volverUrgentes}>Urgentes</button>
    ...
    <main>
      <section className="stats">
        ...
        <div className="stat red"><b>{alertas.length}</b><span>Urgentes ≤ 10 días</span></div>
      ...
      <nav>
        <button className={tab==='alertas'?'active':''} onClick={()=>setTab('alertas')}>Urgentes</button>
        <button className={tab==='productos'?'active':''} onClick={()=>setTab('productos')}>Productos</button>
        <button className={tab==='roturas'?'active':''} onClick={()=>setTab('roturas')}>Roturas</button>
        <button className={tab==='importar'?'active':''} onClick={()=>setTab('importar')}>Importar</button>
      ...
      {tab === 'alertas' && <section className="panel urgent-panel">
        <h2>Urgentes · Caducidad en 10 días o menos</h2>
        <p className="muted">Esta es la pantalla principal. El trabajador revisa, retira producto antiguo y actualiza la nueva fecha.</p>
        <Tabla productos={alertas} openEdit={openEdit} marcarRotura={marcarRotura} eliminarProducto={eliminarProducto} />
      </section>}
      ...
      {tab === 'importar' && <section className="panel">
        ...
        <p className="muted">Puedes pegar listado si hay productos nuevos o cambios.</p>
        <div className="grid">
      ...
      {tab === 'admin' && isAdmin && <section className="panel">
        ...
        <div className="grid two">
          ...
          <div>
            ...
            <p className="muted small">Solo usuario y contraseña. El trabajador no necesita email.</p>
            <input placeholder="Usuario del trabajador" value={newWorker.username} onChange={e=>setNewWorker({...newWorker,username:e.target.value})} />
        ...
        <h3>Usuarios</h3>
        <table><thead><tr><th>Usuario</th><th>Rol</th><th>Centro</th></tr></thead><tbody>{usuarios.map(u=><tr key={u.id}><td>{u.email?.replace('@korvantis.local','')}</td><td>{u.role}</td><td>{u.centros?.nombre || 'Todos'}</td></tr>)}</tbody></table>
      </section>}
  ...
  return <div className="table-wrap"><table>
    ...
    <tbody>{productos.length ? productos.map(p => {
      ...
      return <tr key={p.id}>
        ...
        <td><span className={`badge ${p.estado === 'rotura' ? 'rotura' : d <= 10 ? 'alerta' : 'ok'}`}>{p.estado === 'rotura' ? 'Rotura' : d <= 10 ? 'Urgente' : 'Activo'}</span></td>
    ...
    }) : <tr><td colSpan="8">Sin datos urgentes</td></tr>}</tbody>
  </table></div>
}
