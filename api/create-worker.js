...
export default async function handler(req,res){
 try{
  ...
  const {data:created,error:authError}=await admin.auth.admin.createUser({email,password,email_confirm:true,user_metadata:{usuario,nombre}})
  if(authError)return res.status(400).json({error:authError.message})
  const {error:profileInsertError}=await admin.from("profiles").insert({id:created.user.id,email,nombre,role:"trabajador",centro_id})
  if(profileInsertError)return res.status(400).json({error:profileInsertError.message})
  return res.status(200).json({ok:true})
 }catch(error){return res.status(500).json({error:error.message||"Error interno creando trabajador."})}
}